...
// 首先要获得试卷，这次使用taskNo获得
var questions
window.setTimeout(function(){
	// 获得试卷的内容
    queryPaperByTaskNo(theRequest.taskNo)
},20)   

// 然后获得该同学的答题情况
var returnValue
window.setTimeout(function(){
  queryResultByUsername()
},40) 
...
function queryPaperByTaskNo(n){      
	...
	xmlHttpRequest.onreadystatechange = statechangedQueryPaperByTaskNo;      
  ...
  // 回调函数      
  function statechangedQueryPaperByTaskNo(){
...
function queryResultByUsername(){
  ...
  var url = config.SOCKAddr+"/queryResultByUsername?taskNo="+theRequest.taskNo+"&username="+theRequest.username;          
	...
	xmlHttpRequest.onreadystatechange = statechangedQueryResultByUsername;      
  ...
  // 回调函数      
  function statechangedQueryResultByUsername(){
    var req = xmlHttpRequest;
    if(req.readyState == 4 ){
      if(req.status == 200){
        var json_str = xmlHttpRequest.responseText; // json形式的字符串
 ...
 function showQuesAnswer(quesSequence){
  ...
  if(questions[i].type == 'judgment') {
    ...
    if(returnValue.grades[i][0]=="1"){
        var x1=`<p>他的答案${mapp[returnValue.results[i][0]]}，正确答案${mapp[questions[i].answers[0]]}，回答<p style="color:green">正确:-)</p></p>`
    }else{
        var x1=`<p>他的答案${mapp[returnValue.results[i][0]]}，正确答案${mapp[questions[i].answers[0]]}，回答<p style="color:red">错误:-(</p></p>`
    }
